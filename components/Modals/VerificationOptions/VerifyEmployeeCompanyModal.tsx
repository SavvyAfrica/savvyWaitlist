...
function VerifyEmployeeCompanyModal({setShowVerifyEmployeeCompModal}: MyProps) {
  ...
  const {handleChange, employeeCompanyState, formErrors} = useForm(employeeCompanyForm);
            ...
            <div className="relative p-6 flex-auto">
              ...
              <form className="w-full flex flex-col items-center justify-start mt-8">
                <div className="pb-1.5 mb-1.5 md:w-11/12 w-full">
                  <Label htmlFor='bvn'>Bank Verification Number (BVN)<span className='text-red-600'>*</span></Label>
                  <Input 
                    id='bvn' 
                    name='bvn' 
                    type='tel' 
                    className={` ${formErrors.bvn ? 'border-red-600' : 'border-[#DFDFE6]'} `}
                    value={employeeCompanyState.bvn} 
                    onChange={handleChange}
                  />
                  {formErrors.bvn && <h3 className="text-red-600 text-xs pt-1">{formErrors.bvn}</h3>}
                </div>

                <div className="pb-1.5 mb-1.5 md:w-11/12 w-full">
                  <Label htmlFor='companyRole'>what's your role in the company?<span className='text-red-600'>*</span></Label>
                  <Input 
                    id='companyRole' 
                    name='companyRole' 
                    type='text' 
                    className={` ${formErrors.companyRole ? 'border-red-600' : 'border-[#DFDFE6]'} `} 
                    value={employeeCompanyState.companyRole} 
                    onChange={handleChange}
                  />
                  {formErrors.companyRole && <h3 className="text-red-600 text-xs pt-1">{formErrors.companyRole}</h3>}
                </div>

                <div className="pb-1.5 mb-1.5 md:w-11/12 w-full"> 
                  <Label htmlFor='companyReg'>What's the company's registration number?<span className='text-red-600'>*</span></Label>
                  <Input 
                    id='companyReg' 
                    name='companyReg' 
                    type='text' 
                    className={` ${formErrors.companyReg ? 'border-red-600' : 'border-[#DFDFE6]'} `} 
                    value={employeeCompanyState.companyReg} 
                    onChange={handleChange} 
                  />
                  {formErrors.companyReg && <h3 className="text-red-600 text-xs pt-1">{formErrors.companyReg}</h3>}
                </div>

                <div className="pb-1.5 mb-1.5 md:w-11/12 w-full">
                  <Label htmlFor='companyLoc'>Where is the company located?<span className='text-red-600'>*</span></Label>
                  <Input 
                    id='companyLoc' 
                    name='companyLoc' 
                    type='text' 
                    className={` ${formErrors.companyLoc ? 'border-red-600' : 'border-[#DFDFE6]'} `} 
                    value={employeeCompanyState.companyLoc} 
                    onChange={handleChange} 
                  />
                  {formErrors.companyLoc && <h3 className="text-red-600 text-xs pt-1">{formErrors.companyLoc}</h3>}
                </div>

                <div className="pb-1.5 mb-1.5 md:w-11/12 w-full">
                  <Label htmlFor='validDoc'>Upload a valid registed document<span className='text-red-600'>*</span></Label>                 
                  <input 
                    id='validDoc' 
                    name='validDoc' 
                    type='file' 
                  ...
                  />
                  <p className="mt-1 md:text-sm text-xs text-gray-500 dark:text-gray-300" id="file_input">SVG, PNG, JPG or GIF (MAX. 800x400px).</p>

                  {formErrors.validDoc && <h3 className="text-red-600 text-xs pt-1">{formErrors.validDoc}</h3>}
                </div>
